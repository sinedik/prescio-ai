export const ANALYZE_MARKET_SYSTEM = `You are a prediction market trading analyst.
...
export function buildAnalyzeMarketPrompt(market, eventAnalysis) {
  return `Analyze this prediction market and calculate the trading edge.
...
EVENT CONTEXT:
${eventAnalysis ? `
Situation: ${eventAnalysis.situation_summary}

Key factors:
${(eventAnalysis.key_factors || []).map(f =>
  `- ${f.factor}: ${f.impact} (weight: ${f.weight})`
).join('\n')}

Sentiment: ${eventAnalysis.overall_sentiment}
Uncertainty: ${eventAnalysis.uncertainty_level}
Sources: ${(eventAnalysis.primary_sources || []).map(s => s.name).join(', ')}
...
{
  "fair_prob": <0-100>,
  "edge_score": <exact: fair_prob - ${market.yes_price}>,
  "confidence_score": <0-100>,
  "recommendation": "strong_enter|enter|watch|skip|avoid",
  "kelly_fraction": <0-25>,
  "thesis": "2-3 sentences. WHY is ${market.yes_price}% wrong? Cite specific facts from context.",
